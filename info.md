...
Changes (I need to make a lot of changes, not sure where to start):
  ...
  * floor 2 should have 15 levels before the boss (so wave 25)
    * The pause menu, by defualt, shows you having 1x damage and 1.00x speed. it should be standardized show decimals for both
...
Make enemies more interactive overall:
  ...
  * enemies only spawn on the edge of the game, in the walls. especially in floor two this is a problem because you have to walk all around the stage to find enemies sometimes, they should just spawn anywhere in the level but not too close to the player, and there should be a brief indicator of where they spawn. also the wave scaling in floor two in kind of weird
Item stuff
  * Seemingly none of the floor 2 items currently work as expected. clownish works a bit better, but the nose does not visibly do a honking animation before shrinking back in size, the waves that fire are too fast and not thick enough Mirror Maze works incorrectly. It is supposed to redirect all bullets that hit an enemy to the nearest enemy, currently it seems to only do it with one bulet. Knocking Pins does seem to reduce the contact damage the player takes, but it does not make clippy do any damage when bowing toward enemies. Popcorn bucket does not feel very good to use but I will adress this more after the other items are fixed.
