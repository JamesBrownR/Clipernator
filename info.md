...
Item stuff
  * Seemingly none of the floor 2 items currently work as expected. clownish works a bit better, but the nose does not visibly do a honking animation before shrinking back in size, the waves that fire are too fast and not thick enough. Mirror Maze doesnt seem to work. (to be clear, mirror maze is suposed to work like this: A mirror shard spawns and encircles the player at 80px. when you shoot it, every bullet that touches it redirects to the nearest enemy within 200px. Every bullet that was redirected and kills a enemy, creates a new mirror shard at that enemies death location. When you shoot a mirror shard it priorizizes redirecting to other mirror shards before redirecting to enemies. Also every time a bullet redirects it does 1.5x damage) Knocking Pins works but is too weak (like seriously, it needs to make clippy 5x faster and do 10x the damage in bowling mode. Popcorn bucket does not feel very good to use but I will adress this more after the other items are fixed.
