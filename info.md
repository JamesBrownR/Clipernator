...
Changes (I need to make a lot of changes, not sure where to start):
  ...
  * the wave 11 boss fight is boring, party because the bullet it shoots do not go far enough. The main reason though is it shoots too many bullets at once so it is currently impossible to dodge them by moving in between them, making your only option tanking or staying as far as possible (UPDATE: its possible to dodge now but the boss still shoots too few bullets too slowly)
...
Item stuff
  * Seemingly none of the floor 2 items currently work as expected. clownish works a bit better, but still: only one wave fires instead of two, , the nose does not visusually do a honking animation before shrinking back in size. clown nose does not visually honk when activated. Mirror Maze works incorrectly. It is supposed to redirect all bullets that hit an enemy to the nearest enemy, currently it seems to only do it with one bulet. Kocking Pins does seem to reduce the contact damage the player takes, but it does not make clippy do any damage when bowing toward enemies, and clippy is also far too slow when bowling. Popcorn bucket does not feel very good to use but I will adress this more after the other items are fixed. (UPDATE: Clownish mostly works now, but the waves that fire are too fast, and for some reason it seems to activate constantly after it grows. it is supposed to grow enough to activate and then only activate if there is a enemy within 100px of the player. also it needs to take longer to recharge, and i dont know why it shoots bullets)
