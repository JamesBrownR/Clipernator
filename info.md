...
Item stuff
  * Seemingly none of the floor 2 items currently work as expected. clownish works a bit better, but still: only one wave fires instead of two, , the nose does not visusually do a honking animation before shrinking back in size. clown nose does not visually honk when activated. Mirror Maze works incorrectly. It is supposed to redirect all bullets that hit an enemy to the nearest enemy, currently it seems to only do it with one bulet. Rolling Pins does not seem to reduct the contact damage the player takes by 90%, and it does not make clippy do any damage when bowing toward enemies. Popcorn bucket does not feel very good to use but I will adress this more after the other items are fixed. (UPDATE: Clownish mostly works now, but the waves that fire are too fast, and for some reason it seems to activate constantly after it grows. it is supposed to grow enough to activate and then only activate if there is a enemy within 100px of the player. also it needs to take longer to recharge, and i dont know why it shoots bullets)
